...
class JobItemDetails extends Component {
  ...
  getRandomInterviewer = () => {
    const names = Array.from({length: 60}, (_, i) => `Interviewer ${i + 1}`)
    const interviewer = {
      name: names[Math.floor(Math.random() * names.length)],
      phone: `+91-9${Math.floor(100000000 + Math.random() * 900000000)}`,
      email: `interviewer${Math.floor(Math.random() * 100)}@company.com`,
  ...
  getJobData = async () => {
    ...
    if (response.ok === true) {
      ...
      const interviewer = this.getRandomInterviewer()
      this.setState({
        jobData: updatedData,
  ...
  renderJobDetailsView = () => {
    const {jobData, similarJobsData, interviewer} = this.state
    const {
      companyLogoUrl,
      companyWebsiteUrl,
      employmentType,
        ...
        <div className="job-item">
          ...
          {interviewer && (
            <div className="interviewer-card">
              ...
              <p>Phone: {interviewer.phone}</p>
              <p>Email: {interviewer.email}</p>
              <p>LinkedIn: <a href={interviewer.linkedIn} target="_blank" rel="noreferrer">Profile</a></p>
              <p>Available Slots: {interviewer.timeSlots.join(', ')}</p>
            </div>
          )}
        </div>
